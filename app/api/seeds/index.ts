...
export async function runSeeds(): Promise<void> {
  ...
  try {
    ...
    // Also scan app/models/ for seed files
    const appModelsPath = path.join(process.cwd(), "app", "models");
    const appSeedFiles = await readdir(appModelsPath, { withFileTypes: true }).catch(() => []);
    for (const file of appSeedFiles) {
      if (file.isFile() && seedFilePattern.test(file.name)) {
        const filePath = path.join(appModelsPath, file.name);
        const seedModule = await import(pathToFileURL(filePath).href) as SeedModule;
        seeds.push(...getSeedFunctions(filePath, seedModule));
      }
    }
