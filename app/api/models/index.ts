...
export async function initializeModels(): Promise<void> {
  ...
  // Initialize app-level models (outside modules/)
  const appModelsPath = path.join(process.cwd(), "app", "models");
  const appModelFiles = await readdir(appModelsPath, { withFileTypes: true }).catch(() => []);
  for (const file of appModelFiles) {
    if (file.isFile() && modelFilePattern.test(file.name)) {
      const filePath = path.join(appModelsPath, file.name);
      logger.info(`Initializing app model from ${path.relative(process.cwd(), filePath)}`);
